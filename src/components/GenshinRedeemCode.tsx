...
export const GenshinRedeemCode: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const activeData = await fetchJsonData("genshinRedeemCode", "genshinRedeemCode", "DataRepository");
        const usedData = await fetchJsonData("genshinRedeemCodeUsed", "genshinRedeemCodeUsed", "skson0x6ab_InformationRepository");
                ...
                <TableRow key={index}>
                  ...
                  <TableCell className="text-center">
                    <span
                      onClick={() =>
                        window.open(
                          `https://genshin.hoyoverse.com/gift?code=${item.code}`,
                          "_blank"
                        )
                      }
