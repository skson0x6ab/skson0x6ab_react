...
export const StarRailRedeemCode: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const activeData = await fetchJsonData("starrailRedeemCode", "starrailRedeemCode", "DataRepository");
        const usedData = await fetchJsonData("starrailRedeemCodeUsed", "starrailRedeemCodeUsed", "skson0x6ab_InformationRepository");
  ...
  const markCodeAsUsed = async (code: string) => {
    await readWriteJsonData(code, 'starrailRedeemCodeUsed');
  };
